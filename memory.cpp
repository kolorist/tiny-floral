...
voidptr allocate(const size i_bytes, const size i_alignment, linear_allocator_t* i_allocator)
{
    ...
    // H: header
    // D: data
    // sizeof([A..A]) + sizeof([A'..A']) == i_alignment
    ...
    p8 dataAddr = (p8)align_address(headerAddr + allocHeaderSize, i_alignment); // align forward, this is the start of the data
...
voidptr allocate(const size i_bytes, const size i_alignment, freelist_allocator_t* i_allocator)
{
    ...
    // search
    while (currBlock && !can_fit(currBlock, i_bytes, i_alignment))
    {
    ...
    {
        voidptr dataAddr = align_address((p8)currBlock + allocHeaderSize, i_alignment);
        voidptr* ptrToHeader = (voidptr*)((aptr)dataAddr - sizeof(voidptr));
        // C1: a new free block needs to be created
        if (can_create_new_block(currBlock, i_bytes, i_alignment))
        {
            p8 orgAddr = currBlock->marker;
            p8 dataAddr = (p8)align_aptr((aptr)currBlock + allocHeaderSize, i_alignment); // align forward, this is the start of the data
            size frameSize = (aptr)dataAddr - (aptr)orgAddr + i_bytes;
        ...
        currBlock->alignment = i_alignment;
